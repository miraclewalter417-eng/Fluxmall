        ...
        if (docSnap.exists()) {
          ...
          window.initiateDeposit = async function (amount) {
            if (!document.getElementById("attest").checked) {
              return showToast(
                "For safety sake please Read before proceeding.",
                "warning",
                "ri-close-line",
                "Attestation",
              );
            ...
            if (!amount)
              return showToast(
                "Enter valid amount (Minimum 3000)",
                "error",
                "ri-close-line",
                "Invalid Amount",
              );
            const refCode = Math.floor(
            ...
            try {
              ...
              const config = settingsSnap.exists()
                ? settingsSnap.data()
                : { mode: "manual" };
    ...
    <div class="modal-content">
        ...
        <div class="bank-details">
          ...
          window.payWithKorapay = (amount, key) => {
            ...
            const ref =
              "DEP_" + Date.now() + "_" + Math.floor(Math.random() * 1000);
            ...
            window.Korapay.initialize({
              ...
              onSuccess: function (data) {
                ...
                if (isProcessingDeposit) {
                  console.log(
                    "Prevented double-credit for ref:",
                    data.reference,
                  );
                  return;
                }
          ...
          async function finalizeDeposit(amount, method, refCode, status) {
            ...
            try {
              ...
              if (status === "success") {
                ...
                showToast(
                  `🎉 Success! ₦${amount.toLocaleString()} added to wallet.`,
                  "success",
                  "ri-check-line",
                  "Success",
                );
              ...
              showToast(
                "Error saving deposit: " + err.message,
                false,
                "ri-close-line",
                "Error",
              );
          ...
          window.handleWithdrawalSubmit = async () => {
            ...
            if (!users.bankDetails || !users.bankDetails.accountNumber) {
              return showToast(
                "Please bind your Bank Account in the Profile section first.",
                "warning",
                "ri-close-line",
            ...
            if (amount < globalConfig.minWithdraw) {
              return showToast(
                ` Minimum withdrawal is ₦${globalConfig.minWithdraw.toLocaleString()}`,
                "warning",
                "ri-close-line",
                "Invalid Amount",
              );
...
function startSecurityListeners(uid) {
  ...
  onSnapshot(doc(db, "flash-sales", "auth", "users", uid), (docSnap) => {
    if (docSnap.exists() && docSnap.data().status === "Banned") {
      renderLockScreen(
        "🚫 Account Banned",
        "Your account has been suspended for violating our terms of service. Contact support for appeals.",
      );
    }
  });
  ...
  onSnapshot(
    doc(db, "flash-sales", "auth", "settings", "maintenance"),
    (snap) => {
      if (snap.exists() && snap.data().enabled) {
        renderLockScreen(
          "🛠️ System Maintenance",
          "Our site is currently undergoing scheduled upgrades. We'll be back online shortly!",
        );
      }
    },
  );
...
window.handleSave = async () => {
  ...
  try {
    ...
    const hasExistingBank =
      userData.bankDetails && userData.bankDetails.accountNumber;

    // If System is locked AND this is NOT the first time (it's an edit) -> BLOCK
    if (isMasterLocked && hasExistingBank) {
      return showToast(
        "⛔ Access Denied: The system is currently locked for security. Please contact Admin to change existing details.",
        "warning",
        "ri-close-line",
        "Access Denied",
      );
...
const fetchAmounts = async () => {
  ...
  try {
    ...
    amountListDiv.innerHTML =
      "<p style='color:red;'>Error loading amounts.</p>";
  }
};
...
async function collectDailyEarnings(uid) {
  ...
  for (const docSnap of snap.docs) {
    ...
    const lastClaim =
      share.lastClaimDate?.toDate() || share.purchaseDate.toDate();
    ...
    updates.push(
      updateDoc(
        doc(db, "flash-sales", "auth", "purchased_shares", docSnap.id),
  ...
  await updateDoc(doc(db, "flash-sales", "auth", "users", uid), {
    ib: increment(totalToCredit),
  });
  logActivity(uid, "share", totalToCredit, "Profit");
  showToast(
    `💰 Daily Profit: ₦${totalToCredit.toLocaleString()} added to your balance!`,
    "info",
    "ri-check-line",
    "Profit Added",
  );
...
window.buyShare = async (id, price, name, daily, dur) => {
  ...
  if (bal < price)
    return showToast(
      "Insufficient Balance! try depositing",
      "warning",
      "ri-close-line",
      "Insufficient Balance",
    );
...
function loadMyInvestments(uid) {
  ...
  onSnapshot(
    ...
    (snap) => {
      ...
      if (snap.empty) {
        container.innerHTML =
          "<p style='text-align:center; color:gray;'>No active investments yet.</p>";
      ...
      snap.forEach(async (docSnap) => {
        ...
        const daysPassed = Math.floor(
          Math.abs(now - purchaseDate) / (1000 * 60 * 60 * 24),
        );
        ...
        if (remaining <= 0) {
          try {
            await deleteDoc(
              doc(db, "flash-sales", "auth", "purchased_shares", docId),
            );
            return;
          } catch (err) {
...
onSnapshot(doc(db, "flash-sales", "auth", "settings", "config"), (snap) => {
  if (snap.exists()) {
    ...
    document.getElementById("minLimitTxt").innerText =
      `Minimum withdrawal: ₦${globalConfig.minWithdraw.toLocaleString()}`;
    document.getElementById("withdrawAmount").placeholder =
      `Min: ₦${globalConfig.minWithdraw}`;
    document.getElementById("displayFee").innerText =
      `${globalConfig.withdrawFee}%`;
    if (globalConfig.userAppTheme)
      document.documentElement.setAttribute(
        "data-theme",
        globalConfig.userAppTheme,
      );

    if (globalConfig.referralPercents) {
      document.getElementById("ref1").innerText =
        globalConfig.referralPercents[0] || 0;
      document.getElementById("ref2").innerText =
        globalConfig.referralPercents[1] || 0;
      document.getElementById("ref3").innerText =
        globalConfig.referralPercents[2] || 0;
    }

    // --- LOAD MODAL DATA FROM FIREBASE ---
    const modal = document.getElementById("welcomeModal");
    document.getElementById("siteAbout").innerHTML =
      globalConfig.siteAbout ||
      "Welcome to our premium platform. Founded on trust and speed.";
  ...
  navigator.clipboard
    ...
    .then(() => {
      ...
      if (typeof showAlert === "function") {
        showToast(
          textToCopy + " Copied Successfully",
          "info",
          "ri-check-line",
          "Copied",
        );
...
async function loadTeamData(uid) {
  try {
    ...
    // 4. Update Totals
    document.getElementById("totalTeam").innerText =
      l1Ids.length + l2Count + l3Count;
    ...
    document.getElementById("totalEarnings").innerText =
      `₦${(me.data().refPoints || 0).toLocaleString()}`;
  } catch (err) {
...
window.handleDailyCheckIn = async () => {
  ...
  try {
    ...
    if (!userData.hasDeposited) {
      showToast(
        "⚠️ Please make your first deposit to unlock daily check-in rewards!",
        "warning",
        "ri-close-line",
        "No Deposit",
      );
    ...
    showToast(
      "🎉 Success! " + "" + reward + " " + "points added to your account.",
      "info",
      "ri-check-line",
      "Check-in Reward",
    );
