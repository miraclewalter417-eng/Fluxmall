...
if (signupForm) {
	signupForm.addEventListener('submit', async (event) => {
		...
		let isValid = true;
		if (!validateInput(usernameInput, usernameError, 'Username is required.')) isValid =
			false;
		if (!validateEmail(emailInput, emailError)) isValid = false;
		if (!validateInput(passwordInput, passwordError, 'Password is required.')) isValid =
			false;
		if (usernameInput && (usernameInput.value.length < 2 || usernameInput.value.length >
			15 ||
		...
		if (isValid) {
			try {
				...
				const referralCode = referralCodeInput.value === "" ? 'flash sales' :
				...
				sendEmailVerification(user)
					...
					.catch(error => showAlert("Failed to send verification email.", false));
				signupForm.reset();
				showPage("login-page");
...
if (usernameInput) usernameInput.addEventListener('blur', () => validateInput(usernameInput, usernameError,
	'Username is required.'));
if (emailInput) emailInput.addEventListener('blur', () => validateEmail(emailInput, emailError));
if (passwordInput) passwordInput.addEventListener('blur', () => {
	validateInput(passwordInput, passwordError, 'Password is required.');
	if (passwordInput.value.length < 6 && passwordInput.value.length > 0) passwordError
		.textContent =
...
if (loginForm) {
	loginForm.addEventListener('submit', async (event) => {
		...
		let isValid = true;
		if (!validateInput(loginEmailInput, loginEmailError, 'Email is required.')) isValid =
			false;
		if (!validateInput(loginPasswordInput, loginPasswordError, 'Password is required.'))
			isValid =
				false;
...
if (loginPasswordInput) {
	loginPasswordInput.addEventListener('blur', () => {
		validateInput(loginPasswordInput, loginPasswordError, 'Password is required.');
	});
}
